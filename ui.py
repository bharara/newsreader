...
def datePickerRow(lb):
    ...
    selected_date = col1.date_input("Select a date", max_value=date.today(), on_change=utils.dateChanged, key='selected_date')
...
def table(df, lb):
    ...
    for x, title in enumerate(df['title']):
        ...
        if do_action:
            ...
            story = Story(df["url"][x], df["title"][x], df["category"][x], df["published"][x], df["content"][x], df["summary"][x])
